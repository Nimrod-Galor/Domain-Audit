...
function generateRedirectAnalysis(externalLinks) {
  ...
  for (const [url, data] of Object.entries(externalLinks)) {
    if (data.redirectChain && data.redirectChain.chain.length > 1) {
...
function generateMobileFriendlinessAnalysis(pageData) {
  ...
  for (const [url, data] of Object.entries(pageData)) {
    if (data.mobileFriendliness) {
      ...
      if (mobile.viewport.isResponsive) mobileMetrics.responsiveViewport++;
      if (mobile.mobileFeatures.hasTouchIcons) mobileMetrics.hasTouchIcons++;
      if (mobile.mobileFeatures.hasMediaQueries) mobileMetrics.hasMediaQueries++;
      if (mobile.mobileFeatures.hasFlexboxCSS) mobileMetrics.hasFlexboxCSS++;
      if (mobile.mobileFeatures.hasResponsiveImages) mobileMetrics.hasResponsiveImages++;
