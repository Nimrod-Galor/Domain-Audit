...
import { saveState } from '../src/core/state-manager.js';
...
export async function crawlPage(pageUrl, pendingExternalLinks, {
  ...
  try {
    // Use modular DOM processor for optimized JSDOM creation
    const { dom: domInstance, document: documentInstance, cleanup } = await domProcessor.createDOM(html, pageUrl, {
      loadResources: false,
      keepScripts: false
    });
    
    if (!domInstance || !documentInstance) {
    ...
    const currentPageData = await extractPageDataOptimized(document, html, pageUrl, headers, responseTime, pageSize, statusCode, domInstance);
 ...
 */
async function extractPageDataOptimized(document, html, pageUrl, headers, responseTime, pageSize, statusCode, domInstance) {
