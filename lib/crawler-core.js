...
export async function runExternalChecks(pendingLinksSet, externalLinks, MAX_PARALLEL_CHECKS, MAX_RETRIES, TIMEOUT_MS, MAX_EXTERNAL_LINKS = -1) {
  ...
  if (MAX_EXTERNAL_LINKS > 0 && totalLinks > MAX_EXTERNAL_LINKS) {
    console.log(`⚠️  External link limit applied: checking ${MAX_EXTERNAL_LINKS} out of ${totalLinks} found links`);
  ...
  console.log(`Checking External Links (${totalLinks} links to check${MAX_EXTERNAL_LINKS > 0 && pending.length > MAX_EXTERNAL_LINKS ? ` out of ${pending.length} found` : ''})`);
  ...
  async function worker() {
    while (i < Math.min(pending.length, totalLinks)) {
      ...
      // Log progress for each external link
      console.log(`External Link Check (${currentCount}/${totalLinks}): ${href}`);
      
      const result = await checkExternalLink(href, source, externalLinks, MAX_RETRIES, TIMEOUT_MS);
      
      // Log result for each external link
      if (externalLinks[href]) {
        const status = externalLinks[href].status;
        if (typeof status === 'number') {
          console.log(`External link ${href}: ${status >= 200 && status < 300 ? 'OK' : 'FAILED'} (${status})`);
        } else {
          console.log(`External link ${href}: ${status || 'TIMEOUT'}`);
        }
      }
    }
  }
  ...
  if (linksSkipped > 0) {
    console.log(`External link validation completed: ${processedCount}/${totalLinks} links checked (${linksSkipped} links skipped due to limit)`);
  } else {
    console.log(`External link validation completed: ${processedCount}/${totalLinks} links checked`);
  }
}
...
async function extractPageDataOptimized(document, html, pageUrl, headers, responseTime, pageSize, statusCode, dom) {
  ...
  try {
    ...
    if (analytics.scores) {
      console.log(`📊 Analytics: ${analytics.scores.overall.grade} (${Math.round(analytics.scores.overall.score)}/100) - ${pageUrl}`);
    }
  } catch (analyticsError) {
    console.warn(`Analytics generation failed for ${pageUrl}:`, analyticsError.message);
  }
...
export async function runInternalCrawl(pendingExternalLinks, {
  ...
  console.log(`Starting crawl with ${initialPages} page(s) in queue (limit: ${MAX_INTERNAL_LINKS || 'unlimited'})`);
  ...
  try {
    ...
    if (siteAnalytics) {
      ...
      console.log(`   • Overall Site Grade: ${siteAnalytics.overallGrade}`);
      console.log(`   • Average Score: ${siteAnalytics.averageScore}/100`);
      console.log(`   • Top Issues: ${siteAnalytics.topIssues.length} critical items`);
      console.log(`   • Quick Wins: ${siteAnalytics.quickWins.length} opportunities`);
      console.log(`   • Pages Analyzed: ${siteAnalytics.pagesAnalyzed}`);
    }
  } catch (analyticsError) {
    console.warn(`Site analytics summary generation failed:`, analyticsError.message);
  }
...
export async function generateSiteAnalyticsSummary(pageDataManager, analyticsEngine) {
  try {
    ...
    console.error('Error generating site analytics summary:', error);
    return null;
  }
}
...
export async function exportAnalyticsResults(analyticsResults, outputPath) {
  try {
    ...
    console.log(`Analytics results exported to: ${outputPath}`);
  } catch (error) {
    console.error('Error exporting analytics results:', error);
  }
}
