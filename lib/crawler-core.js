...
export async function runExternalChecks(pendingLinksSet, externalLinks, MAX_PARALLEL_CHECKS, MAX_RETRIES, TIMEOUT_MS) {
  ...
  const totalLinks = pending.length;
  let processedCount = 0;
  let i = 0;

  console.log(`Checking External Links (${totalLinks} links found)`);
...
export async function runInternalCrawl(pendingExternalLinks, {
  ...
  const discoveredPages = queue.size;
  const effectiveMaxPages = Math.min(MAX_INTERNAL_LINKS || Infinity, discoveredPages);
  console.log(`Found ${effectiveMaxPages} pages to crawl (queue: ${discoveredPages}, limit: ${MAX_INTERNAL_LINKS || 'unlimited'})`);
