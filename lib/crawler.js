...
export async function runCrawl(rawDomain, maxInternalLinksArg, forceNew = false, userLimits = {}) {
  ...
  console.log(`  - Max Internal Links: ${MAX_INTERNAL_LINKS}`);
  console.log(`  - Max External Links: ${MAX_EXTERNAL_LINKS === -1 ? 'Unlimited' : MAX_EXTERNAL_LINKS}`);
  console.log(`  - User Registered: ${userLimits.isRegistered ? 'Yes' : 'No'}`);
  ...
  const isWebServerEnv = process.env.SKIP_HTML_REPORT === 'true' || 
                         process.env.WEB_SERVER_MODE === 'true' ||
                         process.env.NODE_ENV === 'production' ||
                         process.env.NODE_ENV === 'development';
  const isWebServerCommand = process.argv[1]?.includes('app.js') ||
                            process.argv[1]?.includes('audit-executor.js') ||
                            process.argv[1]?.includes('jobQueue.js') ||
                            process.argv[1]?.includes('web');
  
  // Default to web server mode unless clearly running as CLI
  const isWebServer = !isDirectExecution || isWebServerEnv || isWebServerCommand;
  
  console.log(`🔍 Execution context: ${isWebServer ? 'WEB_SERVER' : 'CLI'} mode`);
  
  if (isWebServer) {
    // Force cleanup of any hanging HTTP connections in web server mode
    console.log('🧹 Web server mode: cleaning up connections without terminating process');
    setTimeout(() => {
      const activeHandles = process._getActiveHandles?.() || [];
      const activeRequests = process._getActiveRequests?.() || [];
      
      // Close any HTTP sockets that might be keeping the process alive
      ...
      console.log('✅ Connection cleanup completed in web server mode');
    }, 100);
  } else {
    // Only exit in CLI mode when run directly
    console.log('🏁 CLI mode detected - exiting gracefully');
    setTimeout(() => {
      process.exit(0);
    }, 100);
  }
