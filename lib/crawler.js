...
export async function runCrawl(rawDomain, maxInternalLinksArg, forceNew = false) {
  ...
  // Clean up connections in web server mode to prevent hanging
  const isWebServer = process.env.SKIP_HTML_REPORT === 'true' || process.env.NODE_ENV === 'production';
  if (isWebServer) {
    // Force cleanup of any hanging HTTP connections
    setTimeout(() => {
    ...
    // Only exit in CLI mode, not when used as a module in web application
    setTimeout(() => {
      process.exit(0);
    }, 100);
  }
