...
export function generateExternalLinksTable(externalLinks) {
  ...
  for (const [href, data] of Object.entries(externalLinks)) {
    let statusClass = '';
    if (typeof data.status === 'number' && data.status >= 200 && data.status < 300) statusClass = 'status-ok';
    else if (data.status === 'TIMEOUT') statusClass = 'status-timeout';
    else statusClass = 'status-broken';
