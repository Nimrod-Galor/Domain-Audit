...
export class PageTypeClassifier {
  ...
  classifyPage(dom, url = '') {
    try {
      ...
      const classification = {
        ...
        // Analysis components
        urlAnalysis: this._analyzeURL(url),
        titleAnalysis: this._analyzeTitle(document),
        contentAnalysis: this.config.enableContentAnalysis ? this._analyzeContent(document) : null,
        structureAnalysis: this.config.enableStructureAnalysis ? this._analyzeStructure(document) : null,
        semanticAnalysis: this.config.enableSemanticAnalysis ? this._analyzeSemanticMarkup(document) : null,
      ...
      if (classification.detectedTypes.length > 0) {
        ...
        if (primaryMatch.confidence >= this.config.confidenceThreshold) {
  ...
  _analyzeStructure(document) {
    ...
    Object.entries(PAGE_CLASSIFICATION.STRUCTURE_INDICATORS).forEach(([structure, selectors]) => {
      const elements = selectors.reduce((count, selector) => {
        return count + document.querySelectorAll(`[class*="${selector}"], [id*="${selector}"], ${selector}`).length;
      }, 0);
  ...
  _analyzeSemanticMarkup(document) {
    ...
    // Microdata
    const microdataElements = Array.from(document.querySelectorAll('[itemtype]'));
  ...
  _analyzePageCharacteristics(document) {
    const characteristics = {
      ...
      hasSearch: document.querySelectorAll('input[type="search"], [role="search"]').length > 0,
      hasBreadcrumbs: document.querySelectorAll('[class*="breadcrumb"]').length > 0,
      hasPagination: document.querySelectorAll('[class*="pagination"], [class*="pager"]').length > 0,
      hasSocialShare: document.querySelectorAll('[class*="share"], [class*="social"]').length > 0,
      hasComments: document.querySelectorAll('[class*="comment"]').length > 0,
      hasRating: document.querySelectorAll('[class*="rating"], [class*="star"]').length > 0,
      hasCart: document.querySelectorAll('[class*="cart"], [class*="basket"]').length > 0,
      hasPrice: /\$[\d,]+|\£[\d,]+|€[\d,]+|₹[\d,]+/.test(document.body.textContent)
    };
