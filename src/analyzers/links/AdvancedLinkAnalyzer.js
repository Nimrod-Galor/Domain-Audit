...
export class AdvancedLinkAnalyzer extends BaseAnalyzer {
  ...
  getMetadata() {
    return {
      ...
      category: AnalyzerCategories.SEO,
      description: 'Comprehensive link analysis including anchor text patterns, distribution, context, and SEO implications',
      version: '1.0.0',
      author: 'Nimrod Galor',
      tags: ['links', 'anchor-text', 'seo', 'internal-links', 'link-distribution', 'context-analysis'],
      capabilities: [
  ...
  /**
   * Validate input for link analysis
  ...
  validate(context) {
    ...
    if (!context.document && !context.dom) {
      return this.handleError('No document or DOM provided for link analysis');
    }
  ...
  async analyze(context) {
    ...
    try {
      ...
      const analysisResult = await this.performLinkAnalysis(document, url);
      ...
      const recommendations = this._generateLinkOptimizationRecommendations(analysisResult);

      // Generate analysis summary
      const summary = this._generateLinkAnalysisSummary(analysisResult);
      ...
      return this.handleError(`Advanced link analysis failed: ${error.message}`, {
        analyzer: 'AdvancedLinkAnalyzer',
   ...
   * @param {string} url - Base URL for analysis
   * @returns {Object} Comprehensive link analysis results
   */
  async performLinkAnalysis(document, url = '') {
    try {
      const analysis = {
        // Anchor text analysis
        anchorTextAnalysis: this.config.enableAnchorTextAnalysis ? 
          this._analyzeAnchorText(document, url) : null,
  ...
  /**
   * Calculate comprehensive link analysis score for BaseAnalyzer integration
   * @param {Object} analysis - Link analysis results
   * @returns {number} Comprehensive score (0-100)
   */
  _calculateComprehensiveScore(analysis) {
    try {
      const weights = {
        anchorText: 0.30,     // 30% - Anchor text optimization
        context: 0.25,        // 25% - Link context relevance
        distribution: 0.20,   // 20% - Link distribution
        health: 0.15,         // 15% - Overall link health
        structure: 0.10       // 10% - Site architecture
      };
      ...
      // Anchor text score
      if (analysis.anchorTextAnalysis) {
        const anchorData = analysis.anchorTextAnalysis;
        let anchorScore = 60; // Base score

        // Penalize excessive generic anchor text
        const genericPercentage = (anchorData.categories.generic / anchorData.totalLinks) * 100;
        if (genericPercentage > 20) anchorScore -= (genericPercentage - 20);

        // Reward good keyword distribution
        const keywordVariety = Object.keys(anchorData.keywordDensity || {}).length;
        if (keywordVariety > 5) anchorScore += Math.min(keywordVariety * 2, 20);

        // Penalize over-optimization
        const issues = anchorData.issues || [];
        const overOptimizationIssues = issues.filter(i => i.type === 'over_optimization').length;
        anchorScore -= overOptimizationIssues * 10;

        totalScore += Math.max(0, Math.min(anchorScore, 100)) * weights.anchorText;
        totalWeight += weights.anchorText;
      }

      // Context score
      if (analysis.linkContextAnalysis) {
        const contextData = analysis.linkContextAnalysis;
        const relevantPercentage = (contextData.patterns.withRelevantContext / contextData.totalAnalyzed) * 100;
        const contextScore = Math.min(relevantPercentage * 1.2, 100);
      ...
      // Distribution score
      if (analysis.anchorTextAnalysis) {
        const totalLinks = analysis.anchorTextAnalysis.totalLinks;
        const internalLinks = analysis.anchorTextAnalysis.internalLinks;
        
        if (totalLinks > 0) {
          const internalRatio = internalLinks / totalLinks;
          let distributionScore = 50;

          // Optimal internal link ratio is 60-80%
          if (internalRatio >= 0.6 && internalRatio <= 0.8) {
            distributionScore += 30;
          } else if (internalRatio >= 0.4 && internalRatio <= 0.9) {
            distributionScore += 15;
          }

          // Bonus for having links (not too few, not too many)
          if (totalLinks >= 10 && totalLinks <= 100) {
            distributionScore += 20;
          }

          totalScore += Math.min(distributionScore, 100) * weights.distribution;
          totalWeight += weights.distribution;
        }
      }

      // Health score
      const healthScore = analysis.linkHealthScore || 0;
      totalScore += healthScore * weights.health;
      totalWeight += weights.health;

      // Structure score (based on depth analysis)
      if (analysis.depthAnalysis) {
        let structureScore = 70; // Default good score
        
        // Penalize excessive depth
        const avgDepth = analysis.depthAnalysis.averageDepth || 0;
        if (avgDepth > 4) structureScore -= (avgDepth - 4) * 10;
        if (avgDepth > 6) structureScore -= 20; // Additional penalty
        
        totalScore += Math.max(0, Math.min(structureScore, 100)) * weights.structure;
  ...
  /**
   * Generate comprehensive link optimization recommendations
   * @param {Object} analysis - Link analysis results
   * @returns {Array} Enhanced recommendations
   */
  _generateLinkOptimizationRecommendations(analysis) {
    const recommendations = [];
    ...
    try {
      ...
      if (analysis.anchorTextAnalysis) {
        ...
        if (genericPercentage > 15) {
          recommendations.push({
            category: 'anchor-text',
            priority: 'high',
            title: 'Reduce Generic Anchor Text',
            description: `${Math.round(genericPercentage)}% of links use generic anchor text`,
            impact: 'SEO keyword relevance and user experience',
            actionItems: [
              'Replace "click here" and "read more" with descriptive text',
              'Include target keywords in anchor text naturally',
              'Use varied, descriptive anchor text for each link',
              'Ensure anchor text describes the destination page',
              'Avoid over-optimization with exact match keywords'
            ]
          });
        }

        const emptyPercentage = (anchorData.categories.empty / anchorData.totalLinks) * 100;
        if (emptyPercentage > 5) {
          recommendations.push({
            category: 'anchor-text',
            priority: 'medium',
            title: 'Fix Empty Anchor Text',
            description: `${Math.round(emptyPercentage)}% of links have empty anchor text`,
            impact: 'Accessibility and SEO value',
            actionItems: [
              'Add descriptive text to all links',
              'Use alt text for image links',
              'Ensure all links have meaningful anchor text',
              'Review automated link generation processes'
            ]
          });
        }

        // Keyword density issues
        const keywordDensity = anchorData.keywordDensity || {};
        ...
        if (overUsedKeywords.length > 0) {
          recommendations.push({
            category: 'anchor-text',
            priority: 'medium',
            title: 'Diversify Anchor Text Keywords',
            description: `Over-used keywords: ${overUsedKeywords.join(', ')}`,
            impact: 'Keyword cannibalization and over-optimization penalties',
            actionItems: [
              'Use synonyms and related keywords',
              'Vary anchor text for similar pages',
              'Focus on user intent rather than exact keywords',
              'Create natural, diverse linking patterns'
            ]
          });
        }
      }
      ...
      if (analysis.linkContextAnalysis) {
        const contextData = analysis.linkContextAnalysis;
        const irrelevantPercentage = (contextData.patterns.withIrrelevantContext / contextData.totalAnalyzed) * 100;
        ...
        if (irrelevantPercentage > 30) {
          recommendations.push({
            ...
            priority: 'medium',
            title: 'Improve Link Context Relevance',
            description: `${Math.round(irrelevantPercentage)}% of links lack relevant surrounding context`,
            impact: 'User experience and link authority transfer',
            actionItems: [
              'Place links within relevant, descriptive paragraphs',
              'Add contextual information before and after links',
              'Ensure link placement supports content flow',
              'Group related links in logical sections',
              'Use transitional phrases to introduce links'
            ]
          });
        }
      }
      ...
      if (analysis.anchorTextAnalysis) {
        ...
        if (totalLinks < 5) {
          recommendations.push({
            category: 'link-quantity',
            priority: 'low',
            title: 'Increase Internal Linking',
            description: `Only ${totalLinks} links found on this page`,
            impact: 'Site navigation and page authority distribution',
            actionItems: [
              'Add links to related content',
              'Link to important category and product pages',
              'Include navigation links in content',
              'Add contextual links within articles',
          ...
          recommendations.push({
            ...
            description: `${totalLinks} links detected - consider reducing`,
            impact: 'Page load performance and link authority dilution',
            actionItems: [
              'Prioritize most important links',
              'Remove redundant or low-value links',
              'Consolidate similar links',
              'Move secondary links to footer or sidebar',
      ...
      if (analysis.depthAnalysis) {
        ...
        if (avgDepth > 4) {
          recommendations.push({
            ...
            description: `Average page depth is ${avgDepth.toFixed(1)} levels`,
            impact: 'User navigation and search engine crawling',
            actionItems: [
              'Flatten site hierarchy where possible',
              'Add category and hub pages',
              'Implement breadcrumb navigation',
              'Create topic clusters with pillar pages',
              'Add strategic internal links to deep pages'
            ]
          });
        }
      }
      ...
      this.log('Error generating link optimization recommendations:', error.message);
      return [];
    }
  }

  /**
   * Generate comprehensive link analysis summary
   * @param {Object} analysis - Link analysis results
   * @returns {Object} Link analysis summary
   */
  _generateLinkAnalysisSummary(analysis) {
    try {
      const summary = {
        overallHealth: 'Poor',
        totalLinks: 0,
        internalLinkRatio: 0,
        anchorTextQuality: 'Poor',
        contextRelevance: 'Unknown',
        keyFindings: []
      };

      // Total links and ratios
      if (analysis.anchorTextAnalysis) {
        summary.totalLinks = analysis.anchorTextAnalysis.totalLinks;
        if (summary.totalLinks > 0) {
          summary.internalLinkRatio = Math.round(
            (analysis.anchorTextAnalysis.internalLinks / summary.totalLinks) * 100
          );
          summary.keyFindings.push(`${summary.totalLinks} total links, ${summary.internalLinkRatio}% internal`);
        }

        // Anchor text quality assessment
        const genericPercentage = (analysis.anchorTextAnalysis.categories.generic / summary.totalLinks) * 100;
        if (genericPercentage < 10) summary.anchorTextQuality = 'Excellent';
        else if (genericPercentage < 20) summary.anchorTextQuality = 'Good';
        else if (genericPercentage < 35) summary.anchorTextQuality = 'Fair';
        else summary.anchorTextQuality = 'Poor';

        summary.keyFindings.push(`${Math.round(genericPercentage)}% generic anchor text`);
      }
      ...
      if (analysis.linkContextAnalysis) {
        ...
        if (relevantPercentage >= 80) summary.contextRelevance = 'Excellent';
        else if (relevantPercentage >= 60) summary.contextRelevance = 'Good';
        else if (relevantPercentage >= 40) summary.contextRelevance = 'Fair';
        else summary.contextRelevance = 'Poor';

        summary.keyFindings.push(`${Math.round(relevantPercentage)}% contextually relevant links`);
      }

      // Overall health assessment
      const score = this._calculateComprehensiveScore(analysis);
      if (score >= 80) summary.overallHealth = 'Excellent';
      else if (score >= 60) summary.overallHealth = 'Good';
      else if (score >= 40) summary.overallHealth = 'Fair';
      else summary.overallHealth = 'Poor';

      // Additional key findings
      if (analysis.depthAnalysis && analysis.depthAnalysis.averageDepth > 4) {
        summary.keyFindings.push(`Average depth: ${analysis.depthAnalysis.averageDepth.toFixed(1)} levels`);
      }

      if (analysis.anchorTextAnalysis && analysis.anchorTextAnalysis.issues.length > 0) {
        summary.keyFindings.push(`${analysis.anchorTextAnalysis.issues.length} anchor text issues detected`);
      }

      return summary;
    } catch (error) {
      this.log('Error generating link analysis summary:', error.message);
      return {
        overallHealth: 'Unknown',
        totalLinks: 0,
        internalLinkRatio: 0,
        anchorTextQuality: 'Unknown',
        contextRelevance: 'Unknown',
